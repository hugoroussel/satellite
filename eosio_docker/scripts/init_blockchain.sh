...
echo "=== setup wallet: satellitewal ==="
# key for eosio account and export the generated password to a file for unlocking wallet later
cleos wallet create -n satellitewal --to-console | tail -1 | sed -e 's/^"//' -e 's/"$//' > satellite_wallet_password.txt
# Owner key for satellite wallet
cleos wallet import -n satellitewal --private-key 5JpWT4ehouB2FF9aCfdfnZ5AwbQbTtHBAwebRXt94FmjyhXwL4K
# Active key for satellite wallet
cleos wallet import -n satellitewal --private-key 5JD9AGTuTeD5BXZwGQ5AtwBqHK21aHmYnTetHgk1B3pjj7krT8N

# create account for satelliteacc with above wallet's public keys
cleos create account eosio satelliteacc EOS6PUh9rs7eddJNzqgqDx1QrspSHLRxLMcRdwHZZRL4tpbtvia5B EOS8BCgapgYA2L4LJfCzekzeSr3rzgSTUXRXwNi8bNRoz31D14en9
